...
void solve() {
    ...
    if(mx <= 0) {
        ...
        vpii ans2;
        vl b = a;
        int id = max_element(all(a))-begin(a);
        while(a[id] + mn < 0) a[id]+=a[id], ans2.pb({id, id});
        for(int i=0; i<n; i++) {
            if(a[i] < 0) a[i]+=a[id], ans2.pb({i, id});
        }
        for(int i=1; i<n; i++) {
            if(a[i] < a[i-1]) {
                ans2.pb({i, i-1});
        ...
        a = b;
        id = min_element(all(a))-begin(a);
        while(a[id]+mx>0) a[id]+=a[id], ans.pb({id, id});

        for(int i=0; i<n; i++) {
            if(a[i]>0)a[i]+=a[id], ans.pb({i, id});
        }
    ...
    cout << sz(ans) << nl;
    for(auto [x, y]: ans) cout << x+1 << sp << y+1 << nl;
}
int main() {
#ifndef ONLINE_JUDGE
