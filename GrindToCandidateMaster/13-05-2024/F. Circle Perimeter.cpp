...
// read 読む
template<typename T> vector<vector<T>> R(T n, T m) {vector<vector<T>> g(n);rep(i, m) {int u, v; cin >> u >> v;g[--u].pb(--v); g[v].pb(u);}return g;}
...
void solve() {
    /*
     euclidean(0,0)  = sqrt(x^2+y^2)
     -> euclidean(0,0)^2 = x^2+y^2
     -> r<=euclidean(0,0)<(r+1)
     -> r ^ 2 <= x ^ 2 + y ^ 2 < (r+1) ^ 2
    */
    cin >> n;
    ll res = 0, m = n;
    for(ll i=0; i<=n; i++) {
        for(; i*i+m*m>=(n+1)*(n+1);m--){}
        ll u = m; for(; i*i+u*u>=n*n && u;u--,res++){}
    }
    cout << res*4 << nl;
