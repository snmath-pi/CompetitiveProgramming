...
void solve() {
    cin >> n >> m >> k;
    vl a = R(n), b = R(m), c = R(k);

    ll res1 = 0, res2 = 0, id;
    sort(all(a)); repn(i, n-1) {
        chkmax(res2,a[i]-a[i-1]);
        if(res2>res1) swap(res1, res2),id=i;
    }
    ll ita = (a[id]+a[id-1])/2;
    ...
    rep(i, m) { 
        ...
        while(r > l + 1) {
            int mid = l + r >> 1;
            if(b[i] + c[mid] <= ita)l=mid;
            else r = mid;
        }
        chkmin(res1, max({res2, abs(b[i]+c[l]-a[id-1]), abs(b[i]+c[l]-a[id])}));
        chkmin(res1, max({res2, abs(b[i]+c[r]-a[id-1]),abs(b[i]+c[r]-a[id])}));
    }
