...
int main() {
    ...
    int l = *max_element(all(dp));

    deque<pair<int, char>> dq;
    int cnt = 0;
    for(int i=0; i<l; i++) {
        if(!dq.empty() && s[i]==')'&&dq.bk.se =='(')dq.pop_back();
        else dq.pb(mpr(i, s[i]));
    }
    if(dq.empty()) cnt++;
    for(int i=1; i<n-l+1; i++) {
        if(!dq.empty() && dq.ft.fi==i-1)dq.pop_front();
        if(!dq.empty() && s[i+l-1]==')'&&dq.bk.se =='(')dq.pop_back();
        else dq.pb(mpr(i+l-1, s[i+l-1]));
        if(dq.empty()) cnt++;
