...
// Reading
template<typename T> vector<vector<T>> readg(T n, T m) {
    vector<vector<T>> g(n);
    rep(i, m) {
        int u, v; cin >> u >> v;
        g[--u].pb(--v); g[v].pb(u);
...
template<typename T> vector<vector<T>> readgD(T n, T m) {
    vector<vector<T>> g(n);
    rep(i, m) {
        int u, v; cin >> u >> v;
        g[--u].pb(--v);
...
template<typename T> vector<vector<pair<T, T>>> readgW(T n, T m) {
...
template<typename T> vector<T> rea(T n) {
    vector<T> a(n);
    rep(i, n) cin >> a[i];
    return a;
}
...
struct segtree {
    ...
    int max_e(int x, int l, int r, int lx, int rx) {
        if(lx >= r || rx <= l) return -2e9;
        if(lx>=l && rx<=r) return tree[x];
        int m = lx + rx >> 1;
        return max(max_e(2*x+1, l, r, lx,m), max_e(2*x+2, l, r, m, rx));
    }
    int max_e(int l, int r) {
        return max_e(0, l, r, 0, n);
    }
};
void solve() {
    int n;
    ...
    rep(i, n) if(a[i] < b[i]) {yesnoc(0);return;}
    ...
    debug(N)
    bool ok = 1;
    tsi to_change; rep(i, n) {
        if(a[i]!=b[i])to_change.insert(b[i]);
    }

    for(const auto &x: to_change) {
        ok &= (N[x]<=mp[x]);
    }
    yesnoc(ok);
}
