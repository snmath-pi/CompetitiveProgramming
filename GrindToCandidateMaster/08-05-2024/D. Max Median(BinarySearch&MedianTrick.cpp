...
// Reading
template<typename T> vector<vector<T>> readg(T n) {
    vector<vector<T>> g(n);
    rep(i, n-1) {
        int u, v; cin >> u >> v;
        g[--u].pb(--v); g[v].pb(u);
...
template<typename T> vector<vector<T>> readgD(T n) {
    vector<vector<T>> g(n);
    rep(i, n-1) {
        int u, v; cin >> u >> v;
        g[--u].pb(--v);
...
int main() {
    ...
    auto good = [&](int val) {
        vl b(n+1);
        rep(i, n) {
            b[i+1]=b[i] + (a[i] >= val ? 1 : -1);
        }

        vl mn(n+1, n+1); mn[0]=0;
        mn[1] = b[1];
        for(int i=2;i<=n;i++) mn[i] = min(b[i], mn[i-1]);
        ll sum = b[k];
        for(int i=k+1; i<=n; i++) {
            chkmax(sum, b[i] - min(0ll, mn[i-k]));
        }
        return sum > 0;
