...
// Reading
template<typename T> vector<vector<T>> R(T n, T m) {
    vector<vector<T>> g(n);
    rep(i, m) {
        int u, v; cin >> u >> v;
        g[--u].pb(--v); g[v].pb(u);
...
template<typename T> vector<vector<T>> RDG(T n, T m) {
    vector<vector<T>> g(n);
    rep(i, m) {
        int u, v; cin >> u >> v;
        g[--u].pb(--v);
...
template<typename T> vector<vector<pair<T, T>>> RGW(T n, T m) {
...
template<typename T> vector<T> R(T n) {
    vector<T> a(n);
    rep(i, n) cin >> a[i];
    return a;
}
...
void calc() {
    ...
    for(ll i=maxn-2;i>=0;i--)ifact[i]=ifact[i+1]*(i+1);
}
...
int main() {
    ...
    rep(i, q) {
        ...
        if(c == '+') for(int j=k;j>=val;j--) dp[j]+=dp[j-val];
        if(c=='-') for(int j=val;j<=k;j++) dp[j]-=dp[j-val];
        cout << dp[k] << nl;
    }
